...
float sampleDensity( vec4 pos ) {
    ...
    float freq1 = cnoise( vec4(xyz.xy, 1.0, 1.0) );

    // Shader Generated 3D Perlin Noise Texture:
    //float tex1 = cnoise( vec4(xyz.xy, 1.0, 1.0) );
    //float tex2 = cnoise( 4.0 * vec4(xyz.xy, 1.0, 1.0) );
    //float tex3 = cnoise( 8.0 * vec4(xyz.xy, 1.0, 1.0) );
    //float tex_combined = tex1 * (2/3) + tex2 * (1/2) + tex3 * (1/4);
    //float tex_combined = tex1 + tex2 + tex3;
    //tex_combined = tex_combined;

    // Generate higher frequency worley using same texture
    //float freq1 = texture( u_noise, ( xyz.xy + 0.5 ) ).g;

    float freq2 = cnoise( 4.0 * vec4(xyz.xy, 4.0, 3.0) );
    float freq3 = cnoise( 16.0 * vec4(xyz.xy, 2.0, 4.0) );
    ...
    float shape_noise = freq1 * (2/3) + freq2 * (1/2) + freq3 * (1/4);
    shape_noise = shape_noise - 1;
    shape_noise = scale( freq1, shape_noise, 1, 0, 1 );
...
void main() {
    vec3 xyz = v_position.xyz * u_cloud_scale * 0.1 + u_cloud_offset * 0.1;
    //float d = sampleDensity( v_position );
    float c = cnoise( vec4(xyz.xy, 1.0, 1.0) );
    out_color = vec4( 1, 1, 1, c );
