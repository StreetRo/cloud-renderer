...
void main() {
    // vec3 ray_orig = vec3( v_position );
    vec3 ray_orig = u_cam_pos;
    // vec3 ray_dir = u_rt_screen_norm;
    vec3 ray_dir = normalize( vec3( v_position ) - u_cam_pos );
    vec2 distances = rayBoxDst( u_bbox_min, u_bbox_max, ray_orig, ray_dir );

    float val = distance( vec3( v_position ), u_cam_pos );
